...
export const Users: React.FC = () => {
    ...
    const fetchUsers = async () => {
      try {
        ...
        if (err instanceof Error) {
          ...
          setError(
            "Wystąpił nieoczekiwany błąd podczas pobierania użytkowników.",
          );
        }
      } finally {
        setLoading(false);
  ...
  if (loading)
    return (
      <div style={{ padding: "20px" }}>Ładowanie listy użytkowników...</div>
    );
  if (error)
          ...
          {profiles.length === 0 ? (
            ...
            profiles.map((profile) => (
              <tr key={profile.id} style={{ borderBottom: "1px solid #eee" }}>
                <td
                  style={{ padding: "10px", fontSize: "0.85em", color: "#555" }}
                >
                  {profile.id}
                </td>
                ...
                <td style={{ padding: "10px" }}>
                  <span
                    style={{
                      padding: "3px 8px",
                      backgroundColor:
                        profile.role === "ADMIN" ? "#dc3545" : "#17a2b8",
                      color: "white",
                      borderRadius: "12px",
                      fontSize: "0.9em",
                    }}
